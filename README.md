...
| 客户库 | 全列表格（含**客户类型分级列**、邮箱有效性徽章）+ 详情抽屉（阶段/标签/跟进日期/备注时间线/**不再联系开关**）+ 筛选/排序/分页/导出 Excel + **一键验证邮箱（MX）/一键 ICP 分级** + 勾选后触达操作条（模板按客户国家推荐语言、可换附件、可命名 Campaign） |
...
## 测试
- 后端：`cd backend && python -m pytest`（204 用例）
- 前端冒烟：启动后端后 `cd frontend && npx playwright test`（10 用例，只读+勾选，绝不触发真实发送）
